...
@section('title', __('admin.add_research_service'))

@section('content_header')
    <div class="d-flex align-items-center justify-content-between mb-4 mt-2 px-md-4">
        <h1 class="m-0 text-dark font-weight-bold">{{ __('admin.add_research_service') }}</h1>
...
<div class="container-fluid pb-5">
    <div class="row justify-content-center px-md-4">
        <div class="col-12">
            <form action="{{ route('admin.research-services.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                            ...
                            <div class="card-body p-4">
                                <div class="form-group mb-4">
                                    ...
                                    <input type="text" name="title" class="form-control form-control-lg @error('title') is-invalid @enderror shadow-none bg-light border-0 px-3 py-4" 
                                           placeholder="Masukkan judul..." value="{{ old('title') }}" required style="border-radius: 12px; font-size: 1.1rem;">
                                    ...
                                    <div class="col-md-8">
                                        <div class="form-group mb-0">
                                            ...
                                            <input type="text" name="author" class="form-control @error('author') is-invalid @enderror shadow-none bg-light border-0 px-3 py-3" 
                                                   value="{{ old('author') }}" placeholder="Nama Dosen / Tim Pelaksana..." style="border-radius: 10px;">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group mb-0">
                                            ...
                                            <input type="text" name="year" class="form-control @error('year') is-invalid @enderror shadow-none bg-light border-0 px-3 py-3" 
                                                   value="{{ old('year', date('Y')) }}" placeholder="202X..." style="border-radius: 10px;">
                                ...
                                <div class="form-group mb-4">
                                    ...
                                    <textarea name="abstract" class="form-control shadow-none bg-light border-0 px-3 py-3" rows="4" 
                                              placeholder="Ringkasan singkat atau abstrak..." style="border-radius: 12px;">{{ old('abstract') }}</textarea>
                                </div>

                                <div class="form-group mb-0">
                                    <label class="form-label font-weight-bold small text-muted text-uppercase mb-2">{{ __('admin.research_content') }}</label>
                                    <textarea name="content" class="form-control content-editor">{{ old('content') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card shadow-sm border-0 mb-4" style="border-radius: 16px; overflow: hidden;">
                            ...
                            <div class="card-body p-3">
                                <div class="form-group mb-4">
                                    ...
                                    <select name="type" class="form-control shadow-none bg-light border-0" style="border-radius: 8px;">
                                        <option value="research" {{ old('type') == 'research' ? 'selected' : '' }}>{{ __('admin.type_research') }}</option>
                                        <option value="community_service" {{ old('type') == 'community_service' ? 'selected' : '' }}>{{ __('admin.type_community_service') }}</option>
                                    </select>
                                </div>
                                ...
                                <div class="form-group mb-4">
                                    ...
                                    @include('admin.partials.media-input', ['name' => 'featured_image', 'label' => false, 'value' => old('featured_image')])
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="url" name="external_link" class="form-control form-control-sm border-0 bg-light shadow-none" 
                                           placeholder="https://doi.org/..." value="{{ old('external_link') }}" style="border-radius: 8px;">
                                </div>
                            </div>
                            <div class="card-footer bg-light p-3 border-0">
                                <button type="submit" class="btn btn-primary btn-block shadow font-weight-bold rounded-pill mb-2 py-2">
                                     <i class="fas fa-save mr-2"></i> {{ __('admin.save_research_service') }}
                                </button>
...
<script>
    $(document).ready(function() {
        ...
        // TinyMCE Editor - Premium Modern Config (matches your global style)
        tinymce.init({
        ...
        // Hide "Upgrade" button and clean up UI
